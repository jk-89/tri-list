...
int main() {
    ...
    for (auto i : l.range_over<float>()) {
        ...
        s << i - std::numbers::pi_v<float>;
        assert(s.str() == "0.858407");
    }
    ...
    std::stringstream s;
    std::ranges::copy(l2.range_over<std::string>(), std::ostream_iterator<std::string>(s, ", "));
    assert(s.str() == "This, is, a, string, ");
    ...
    assert(std::ranges::count(l3.range_over<int>(), 1) == 3);
